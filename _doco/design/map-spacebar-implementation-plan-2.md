...
- This is document version 2.  It has been edited by the Design team, providing answers to questions, and culling options. The version 1 file can be found here: `_doco/design/map-spacebar-implementation-plan.md`
...
Approved: `setSpacebarAction` is added. It should store an existing action string such as `nextBeat` or `songReset`, not a separate mapping id.
...
TODO: Resolve why action would not be stored in "action" property.  Would that mean that key-handler.js would try to implement the action directly, skipping a step???
