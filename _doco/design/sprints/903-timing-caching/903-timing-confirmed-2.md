...
This capture adds direct, measured support for the fix already proposed as a "pure win regardless of caching strategy" in [903-timing-revisited.md](903-timing-revisited.md#other-caching-choices-to-consider-design-level-not-yet-decided): skip the `buildCellsForTable()` call inside `buildCells()`/`resetNoteNames()` for any table whose wiring has a non-empty `relativeSection`, since `replay()` immediately rebuilds it correctly afterward. Based on this capture, that alone would remove ~46ms × 2 tables ≈ 92ms from every `buildCells()` cycle (roughly 33-45% of its 182-271ms measured duration) with zero risk of stale output, independent of and prior to any whole-table-cache redesign.

## Applied

Implemented in [buildCells()](../../../../infinite-neck.js#L1070-L1090): tables whose `song.wirings` entry has a non-empty `relativeSection` are now skipped in the `buildCells()` pass, since `resetNoteNames()` always calls `replay()` immediately afterward, and `replayTable()`'s `RELATIVE` branch ([NoteTableController.js](../../../../NoteTableController.js#L1108-L1118)) unconditionally rebuilds that same table with the correct relative-section options. `buildCells()`/`resetSharps()`/`resetFlats()` have no other callers, so `replay()` always runs right after — no stale-output risk. Full Jest suite validated after the change: 65 suites / 695 tests passing.

The larger, not-yet-decided option from this document — caching and injecting whole-table markup with a single batched DOM write per table instead of the current per-note-class `.each()` loop of individual `.html()`/`.css()` calls — remains a design-level decision and was not implemented as part of this fix.
