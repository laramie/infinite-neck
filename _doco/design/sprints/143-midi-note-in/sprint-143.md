...
## Iterations
  ...
  - "Iteration 4": 
    ...
    - "Iteration 5": VoiceLive 3 Integration
      ...
      - Lots of Rounds, but notably Round 9 is where we got the note forwarding right for Momentary/Latch/Named/Single/Highhlight, so you can really play the Launchpad and the note on/off get forwarded perfectly to Voicelive 3.  There are some interesting artifacts of some of the combinations, but they are logically correct.  Notably, with NamedNote present in Section, pressing and releasing on button board sends and releases to Voicelive, and in Latch mode clears a NamedNote, which is logical and fine.  Latch mode is for recording notes and capturing what you might be playing, and is a bit funny with NamedNotes in this way, but matches the UI mouse behavior, per design.